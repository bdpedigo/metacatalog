...
### Requirement: Step 2 — Spec name field
Each spec SHALL have a `name` field that determines the output folder name in the destination path. The `name` field SHALL be independently editable and is NOT coupled to `partition_by`. For auto-discovered specs, `name` defaults to the `partition_by` value (e.g., `id` for a primary-key spec, `ctr_morton` for a spatial spec). The user MAY change `name` without affecting `partition_by`, and vice versa.

#### Scenario: Default name matches partition_by after discovery
- **WHEN** auto-discovery produces a spec with `partition_by = "id"`
- **THEN** the system SHALL set `name = "id"` as the default output folder name

#### Scenario: Default name for spatial spec
- **WHEN** auto-discovery produces a spatial spec with `partition_by = "ctr_morton"`
- **THEN** the system SHALL set `name = "ctr_morton"` as the default output folder name

#### Scenario: User edits name independently
- **WHEN** the user changes `name` to `"by_position"` on a spec with `partition_by = "ctr_morton"`
- **THEN** the system SHALL use `"by_position"` as the output folder name while `partition_by` remains `"ctr_morton"`

### Requirement: Step 2 — Spatial column display in dropdowns
Partition and z-order column dropdowns SHALL hide internal computed columns (`_morton`, `_x`, `_y`, `_z` suffixes of geometry columns) and instead show the original geometry column names. Geometry column names SHALL be displayed with a `*` suffix (e.g., `ctr *`) to indicate they are spatial columns that will be morton-encoded in the output. The internal storage SHALL continue to use the `_morton` form (e.g., `ctr_morton`) for both `partition_by` and `zorder_columns`.

#### Scenario: Partition dropdown hides internal columns
- **WHEN** a table has geometry column `ctr` producing computed columns `ctr_morton`, `ctr_x`, `ctr_y`, `ctr_z`
- **THEN** the partition dropdown SHALL show `ctr *` and hide `ctr_morton`, `ctr_x`, `ctr_y`, `ctr_z`

#### Scenario: Z-order dropdown hides internal columns
- **WHEN** a table has geometry column `ctr` and `ctr_morton` is not yet in the z-order list
- **THEN** the z-order dropdown SHALL show `ctr *` and hide `ctr_morton`, `ctr_x`, `ctr_y`, `ctr_z`

#### Scenario: Z-order badge shows friendly name
- **WHEN** a z-order column is internally stored as `ctr_morton`
- **THEN** the z-order badge SHALL display `ctr *`

### Requirement: Step 2 — Spatial partition column selection
When the user selects a spatial (geometry) column for partitioning, the system SHALL auto-set `partition_by` to `{col}_morton` and `partition_strategy` to `uniform_range`. The system SHALL display a tooltip explaining the morton encoding derivation (e.g., "Spatial column — will be morton-encoded → `ctr_morton`"). Partition and z-order are independent — selecting a spatial column for partition SHALL NOT automatically modify the z-order column list.

#### Scenario: Selecting a spatial column for partition
- **WHEN** the user selects `ctr *` in the partition dropdown
- **THEN** the system SHALL set `partition_by = "ctr_morton"`, `partition_strategy = "uniform_range"`, and display the morton encoding tooltip

#### Scenario: Partition selection does not affect z-order
- **WHEN** the user selects `ctr *` in the partition dropdown and z-order is currently `["id"]`
- **THEN** z-order SHALL remain `["id"]` — no automatic modification

#### Scenario: Tooltip for auto-discovered spatial spec
- **WHEN** Step 2 loads with an auto-discovered spatial spec that has `source_geometry_column = "ctr"`
- **THEN** the system SHALL display the morton encoding tooltip showing `partition_by = "ctr_morton"`

### Requirement: Step 2 — Spatial z-order column selection
When the user selects a spatial (geometry) column for z-ordering, the system SHALL internally store `{col}_morton` in the z-order list but display the friendly name with `*` suffix in the badge. When any z-order column is a morton-encoded spatial column, the system SHALL display a tooltip explaining that spatial columns (marked `*`) are morton-encoded.

#### Scenario: Adding a spatial column to z-order
- **WHEN** the user selects `ctr *` from the z-order dropdown
- **THEN** the system SHALL add `ctr_morton` to `zorder_columns` and display it as `ctr *` in the badge list

### Requirement: Step 2 — Spec review and editing
The system SHALL display each discovered `DeltaLakeOutputSpec` as an editable card showing: `name` (output folder name), `partition_by`, `partition_strategy`, `n_partitions` (with override input), `target_file_size_mb` (optional override, inheriting from global), `zorder_columns`, `bloom_filter_columns`, and `source_geometry_column` (if spatial). The system SHALL also display table metadata (row count, estimated bytes/row) as read-only context.
